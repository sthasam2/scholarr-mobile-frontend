...
class ClasscontentClient {
  ...
  Future<dynamic> getSubmissionDetail(int submission_id) async {
    try {
      ...
      final url2 = Uri.parse(urls.submissionPlagiarismURL(submission_id));
      ...
      if (response1.statusCode < 400 && response2.statusCode < 400) {
        final SubmissionItemDetail response_body1 =
            SubmissionItemDetail.fromJson(jsonDecode(response1.body));
        final submission =
            SubmissionListItem.fromJson(response_body1.submission);
        final attachments = (response_body1.attachments!.isEmpty)
            ? []
            : response_body1.attachments!
                .map((attachment) => AttachmentItem.fromJson(attachment))
                .toList();

        final PlagiarismList response_body2 =
            PlagiarismList.fromJson(jsonDecode(response2.body));
        final plagiarism_list = (response_body2.plagiarism!.isEmpty)
            ? []
            : response_body2.plagiarism!
                .map((item) => PlagiarismListItem.fromJson(item))
                .toList();

        return {
          "submission": submission,
          "attachments": attachments,
          "plagiarism_list": plagiarism_list,
        };
      } else {
        return Error.fromJson(jsonDecode(response1.body));
      }
    } on SocketException {
