...
class ClasscontentManager extends ChangeNotifier {
  ...
  //
  // SETTERS
  void setDetailClasscontentTypeAsClasswork() {
    _detailClasscontentType = "CLASSWORK";
  }
  ...
  void setSelectedSubmissionDetail() async {
    if (_detailClasscontentType! == "SUBMISSION" &&
        _selectedSubmissionIndex != -1) {
      final submissionDetail = await _classcontentClient
          .getSubmissionDetail(_selectedSubmissionIndex);

      _selectedClassworkItemDetail = submissionDetail;
    }

    debugPrint("Classcontent Done");
  }

  Future<Map> getSelectedSubmissionDetail() async {
    if (_detailClasscontentType! == "SUBMISSION" &&
        _selectedSubmissionIndex != -1) {
      final submissionDetail = await _classcontentClient
          .getSubmissionDetail(_selectedSubmissionIndex);
